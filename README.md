...
## Implementation notes and source declaration

This submission was implemented with original HTML, SCSS, and JavaScript and does not use a front-end library. It uses the following external resources:

- **Typography:** Google Fonts serves the Manrope and DM Mono webfonts.
- **Video:** The HTML5 video demonstration uses MDN's CC0 flower video (`interactive-examples.mdn.mozilla.net`). The custom poster art is included in this repository.
- **Department link and social destinations:** Footer links point to public Ohio State CSE and university social pages. No code was copied from those sites.
- **Development assistance:** OpenAI Codex was used to generate and refine the implementation; the required interaction log is included as `llm_logs.csv`.
